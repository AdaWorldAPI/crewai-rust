//! Bridge between crewai-rust local contract types and `ladybug-contract` V1 types.
//!
//! The V1 types in `ladybug-contract` are the canonical wire-format types shared
//! across all three runtimes. This module provides zero-cost `From` conversions
//! so that crewai-rust can emit / ingest the canonical format while keeping its
//! richer local types (with UUID constructors, lifecycle helpers, etc.).
...
mod tests {
    ...
    fn test_unified_step_roundtrip() {
        ...
        assert_eq!(back.step_type, "crew.agent");
    ...
    fn test_delegation_roundtrip() {
        let step = UnifiedStep::new("e1", "crew.agent", "Test", 0);
        let input = DataEnvelope::new(json!({"query": "rust"}), "trigger");
