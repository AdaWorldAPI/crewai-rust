...
mod tests {
    ...
    #[test]
    fn test_dimension_names_are_valid_identifiers() {
        // All dimension names must be lowercase snake_case identifiers.
        for name in &DIMENSION_NAMES {
            assert!(
                name.chars().all(|c| c.is_ascii_lowercase() || c == '_'),
                "dimension '{}' is not a valid snake_case identifier",
                name
            );
            assert!(!name.is_empty(), "dimension name must not be empty");
        }
    }
    ...
    #[test]
    fn test_node_format_uses_agent_namespace() {
