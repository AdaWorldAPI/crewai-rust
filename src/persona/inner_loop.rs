...
mod tests {
    ...
    #[test]
    fn test_default_state_serializes_cleanly() {
        let state = test_state();
        let serialized = serde_json::to_string(&state).unwrap();
        // Default state should serialize without error and contain expected keys
        assert!(serialized.contains("thinking_style"));
