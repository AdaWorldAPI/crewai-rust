...
mod tests {
    ...
    fn test_no_identity_references() {
        // Verify YAML contains no identity-specific content
        for facet in Facet::ALL {
            let yaml = facet.default_module_yaml().to_lowercase();
            assert!(!yaml.contains("ada"), "{:?} YAML leaks identity", facet);
            assert!(!yaml.contains("nsfw"), "{:?} YAML leaks NSFW", facet);
            assert!(!yaml.contains("eroti"), "{:?} YAML leaks NSFW", facet);
        }
    }
}
