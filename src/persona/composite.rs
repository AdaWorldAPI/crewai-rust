...
// ============================================================================
// Preset composites (neutral labels — no identity, no NSFW)
// ============================================================================
...
mod tests {
    ...
    #[test]
    fn test_preset_names_are_neutral() {
        for preset in PresetComposite::ALL {
            let name = preset.composite().name;
            let lower = name.to_lowercase();
            assert!(!lower.contains("ada"), "preset '{}' contains identity ref", name);
            assert!(!lower.contains("wife"), "preset '{}' contains identity ref", name);
            assert!(!lower.contains("eroti"), "preset '{}' contains NSFW ref", name);
            assert!(!lower.contains("nsfw"), "preset '{}' contains NSFW ref", name);
        }
    }
